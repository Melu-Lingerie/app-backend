...
class CartClearServiceIntegrationTest extends CartIntegrationTestBase {
    ...
    void shouldClearCartWithMultipleItemsSuccessfully() {
        ...
        // Verify all created items are marked as deleted
        CartItem deletedItem1 = cartItemRepository.findById(item1.getId()).orElseThrow();
        CartItem deletedItem2 = cartItemRepository.findById(item2.getId()).orElseThrow();
        CartItem deletedItem3 = cartItemRepository.findById(item3.getId()).orElseThrow();

        assertTrue(deletedItem1.isDeleted(), "Item1 should be deleted");
        assertTrue(deletedItem2.isDeleted(), "Item2 should be deleted");
        assertTrue(deletedItem3.isDeleted(), "Item3 should be deleted");

        // Verify all items still exist in database but are marked as deleted
    ...
    void shouldClearCartWithSingleItem() {
        ...
        CartItem deletedItem = cartItemRepository.findById(item.getId()).orElseThrow();
    ...
    @Test
    @DisplayName("Should handle clearing cart with already deleted items")
    void shouldHandleClearingCartWithAlreadyDeletedItems() {
        // Given
        Long userId = 42L;
        Cart cart = createTestCart(userId);
        CartItem activeItem = createTestCartItem(cart, 100L, 200L, 2);
        CartItem deletedItem = createTestCartItem(cart, 101L, 201L, 3);
        ...
        // Then
        assertEquals(1, clearedCount); // Only the active item should be counted

        // Verify both items are now deleted
        CartItem item1 = cartItemRepository.findById(activeItem.getId()).orElseThrow();
        CartItem item2 = cartItemRepository.findById(deletedItem.getId()).orElseThrow();
    ...
    void shouldNotAffectOtherCartsWhenClearingSpecificCart() {
        ...
        // Verify cart1's item is deleted
        CartItem deletedItem = cartItemRepository.findById(item1.getId()).orElseThrow();
        assertTrue(deletedItem.isDeleted());
