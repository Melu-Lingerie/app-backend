...
class CartGetServiceIntegrationTest extends CartIntegrationTestBase {
    ...
    @Test
    @DisplayName("Should return updated cart after item removal")
    void shouldReturnUpdatedCartAfterItemRemoval() {
        // Given
        Long userId = 42L;
        Cart cart = createTestCart(userId);

        CartItem activeItem = createTestCartItem(cart, 100L, 200L, 2);
        CartItem itemToDelete = createTestCartItem(cart, 101L, 201L, 1);

        // Remove one item from database (hard delete)
        ...
        assertNotNull(result);
        assertEquals(cart.getId(), result.cartId());
        assertEquals(1, result.itemsCount()); // Only remaining item should be counted
